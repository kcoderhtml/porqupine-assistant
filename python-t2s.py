...
def actionhandeler(intent, slot):
	intents = ['LockPC', 'weather', 'MediaControls']
	actions = ['espeak \'Lockdown Initiated.\' & dbus-send --type=method_call --dest=org.gnome.ScreenSaver /org/gnome/ScreenSaver org.gnome.ScreenSaver.Lock', 'espeak \'Getting the weather.\' && curl wttr.in/?format="%t:+%C" | espeak', 'playerctl %s & espeak \'%s\'' % (slot, slot)]
	if (intent in intents):
		print(intents.index(intent))
		os.system(actions[intents.index(intent)])
...
class PicovoiceDemo(Thread):
    ...
    def _inference_callback(inference):
        if inference.is_understood:
            ...
            print('  }')
            print('}\n')
            actionhandeler(inference.intent, value)
        else:
            print("Sorry. I did not understand. Try again.\n")
            os.system('espeak \'Sorry. I did not understand. Try again.\' &')
